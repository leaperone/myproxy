...
function parseConnection(value: unknown): Connection | null { const item = record(value); if (!item) return null; const id = string(item.id); const host = string(item.host); const port = number(item.port); const network = string(item.network); const outbound = string(item.outbound); const rule = string(item.rule); const chain = parseStrings(item.chain); const uploadBytes = number(item.uploadBytes); const downloadBytes = number(item.downloadBytes); const startedAt = number(item.startedAt); return id !== null && host !== null && port !== null && network !== null && outbound !== null && rule !== null && chain !== null && uploadBytes !== null && downloadBytes !== null && startedAt !== null ? { id, host, port, network, outbound, rule, chain, uploadBytes, downloadBytes, startedAt } : null; }
