...
internal class MyProxyStore(private val context: Context) {
    ...
    private val runtimeFile get() = File(context.filesDir, "myproxy-mobile.runtime.json")
    ...
    @Synchronized fun readRuntime(): String? = runtimeFile.takeIf { it.isFile }?.readText(StandardCharsets.UTF_8)
    @Synchronized fun writeRuntime(runtime: String) { runtimeFile.writeText(runtime, StandardCharsets.UTF_8) }
}
