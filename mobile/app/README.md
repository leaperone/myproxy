...
The mobile app uses Expo SDK 57, React Native 0.87 and React 19.2. The repository does not commit a lockfile from a developer machine. CI installs the exact dependency graph and builds the iOS/Android artifacts remotely.
...
- **节点** expands each group inline, supports manual selection, fallback and latency based groups, and adds either a subscription URL or a share link.
...
The UI is Chinese-first, follows the system light/dark appearance, supports text scaling, and keeps destructive source deletion behind a confirmation. Device and network acceptance are separate from this source milestone; GitHub Actions owns builds to keep developer machines small.
