...
func Validate(renderJSON string) error {
	var cfg render
	if len(renderJSON) > 8*1024*1024 || json.Unmarshal([]byte(renderJSON), &cfg) != nil || len(cfg.Nodes) == 0 {
		return errors.New("代理配置无效")
	}
	config, err := core.LoadConfig("json", bytes.NewReader([]byte(cfg.Config)))
	if err != nil {
		return errors.New("节点配置未通过 Xray 验证")
	}
...
func NewEngine(renderJSON string, policy Policy, protector Protector, allowDirect bool) (*Engine, error) {
	if policy == nil || len(renderJSON) > 8*1024*1024 {
	...
	if process.engine != nil {
		process.Unlock()
		cancel()
		return nil, errors.New("上一个代理尚未停止")
	}
	if !process.registered {
		if err := internet.RegisterDialerController(protectSocket); err != nil {
			process.Unlock()
			cancel()
			return nil, errors.New("无法准备代理网络")
		}
	...
	if host, port, err := net.SplitHostPort(bootstrap); err != nil || net.ParseIP(host) == nil || port != "53" {
		e.Close()
		return nil, errors.New("启动 DNS 地址无效")
	}
	...
	if err != nil {
		e.Close()
		return nil, errors.New("节点配置未通过 Xray 验证")
	}
	e.instance = instance
	...
	if err := e.stack.CreateNIC(1, e.link); err != nil {
		e.Close()
		return nil, fmt.Errorf("无法创建隧道接口: %s", err)
	}
	if err := e.stack.SetPromiscuousMode(1, true); err != nil {
		e.Close()
		return nil, errors.New("无法接入隧道流量")
	}
	if err := e.stack.SetSpoofing(1, true); err != nil {
		e.Close()
		return nil, errors.New("无法准备隧道地址")
	}
...
func protectSocket(_, _ string, raw syscall.RawConn) error {
	...
	if e == nil || e.closed.Load() {
		return errors.New("代理已停止")
	}
	if e.protector == nil {
		return nil
	}
	var rejected bool
	if err := raw.Control(func(fd uintptr) { rejected = !e.protector.Protect(int64(fd)) }); err != nil {
		return err
	}
	if rejected {
		return errors.New("无法保护代理连接")
	}
	return nil
}

func (e *Engine) Start(writer PacketWriter) error {
	if writer == nil || e.closed.Load() || !e.started.CompareAndSwap(false, true) {
		return errors.New("隧道状态无效")
	}
...
func (w *fileWriter) WritePacket(packet []byte) bool {
	n, err := w.file.Write(packet)
	return err == nil && n == len(packet)
}

func (e *Engine) StartTun(fd int64) error {
	if fd < 0 || fd > 1<<30 {
		return errors.New("隧道描述符无效")
	}
	duplicate, err := syscall.Dup(int(fd))
	if err != nil {
		return errors.New("无法打开隧道")
	}
	...
	e.tun = file
	e.mu.Unlock()
	if err = e.Start(&fileWriter{file}); err != nil {
		file.Close()
		return err
	}
	go func() {
		...
		for {
			...
			if err := e.WritePacket(packet[:n]); err != nil && e.closed.Load() {
				return
			}
		}
	}()
...
func (e *Engine) WritePacket(packet []byte) error {
	if e.closed.Load() || !e.started.Load() {
		return errors.New("隧道未连接")
	}
	if len(packet) < 20 || len(packet) > 65535 {
		return errors.New("无效的数据包")
	}
...
func (e *Engine) decide(id stack.TransportEndpointID, network string) (route, string, error) {
	...
	if len(data) > 4096 {
		return route{}, host, errors.New("连接信息过长")
	}
	response := e.policy.Decide(string(data))
	if len(response) > 64*1024 || json.Unmarshal([]byte(response), &reply) != nil || !reply.OK {
	...
	if r.Action != "proxy" || r.Tag == nil || !e.tags[*r.Tag] {
		return r, host, errors.New("没有可用出口")
	}
	return r, host, nil
}
...
func (e *Engine) reserve(id stack.TransportEndpointID, network string, r route, host string) (*flow, context.Context, error) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if e.closed.Load() || len(e.flows) >= maxFlows {
		return nil, nil, errors.New("连接数量达到上限")
	}
...
func (e *Engine) finish(f *flow) {
	...
	if len(e.recent) > maxRecent {
		e.recent = append([]flowView(nil), e.recent[len(e.recent)-maxRecent:]...)
	}
}

func (e *Engine) acceptTCP(request *tcp.ForwarderRequest) {
	id := request.ID()
	r, host, err := e.decide(id, "tcp")
	if err != nil {
		request.Complete(true)
		return
	}
	f, ctx, err := e.reserve(id, "tcp", r, host)
	if err != nil {
		request.Complete(true)
		return
	}
	go func() {
		...
		f.local = local
		e.mu.Unlock()
		stop := context.AfterFunc(ctx, func() { local.Close(); upstream.Close() })
		defer stop()
		done := make(chan struct{})
		go func() {
			copyTraffic(upstream, local, &f.up, &e.up)
...
func (e *Engine) dial(ctx context.Context, r route, host string, port uint16, network string) (net.Conn, error) {
	if r.Action == "direct" {
		if !e.allowDirect {
			return nil, errors.New("当前平台不支持动态直连")
		}
	...
	if r.Tag == nil || !e.tags[*r.Tag] {
		return nil, errors.New("出口不可用")
	}
	ctx = session.SetForcedOutboundTagToContext(ctx, *r.Tag)
	destination := xnet.TCPDestination(xnet.ParseAddress(host), xnet.Port(port))
	if network == "udp" {
		destination = xnet.UDPDestination(xnet.ParseAddress(host), xnet.Port(port))
	}
...
func (e *Engine) Close() {
	...
	if process.engine == e {
		process.engine = nil
		if e.previousResolver != nil {
			net.DefaultResolver = e.previousResolver
		}
	}
	process.Unlock()
}
...
func (e *Engine) Snapshot() string {
	...
	if e.closed.Load() {
		phase = "disconnected"
	} else if e.started.Load() {
		phase = "connected"
	}
	var since any
	if value := e.startedAt.Load(); value > 0 {
		since = value
	}
