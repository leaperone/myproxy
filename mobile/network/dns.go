...
func (e *Engine) observeDNS(query, response []byte) {
	...
	if q.Unpack(query) != nil || r.Unpack(response) != nil || len(q.Questions) != 1 || len(r.Questions) != 1 || !r.Header.Response || r.Header.RCode != dnsmessage.RCodeSuccess {
		return
	}
	if q.Questions[0] != r.Questions[0] {
		return
	}
	...
	if len(e.names) > 2048 {
		for ip, entry := range e.names {
			if now.After(entry.expires) {
				delete(e.names, ip)
			}
		}
		for ip := range e.names {
			if len(e.names) <= 2048 {
				break
			}
			delete(e.names, ip)
