...
fn service() -> &'static Mutex<Service> { SERVICE.get_or_init(Mutex::default) }
fn active() -> Result<Arc<Runtime>> {
    service().lock().expect("Xray service").runtime.clone().context("尚未连接 Xray")
}
...
pub fn default_strategy() -> Strategy {
    ...
    for (name, patterns) in [
        ...
        strategy.groups.push(Group::matching(name.into(), "fallback".into(), vec![], patterns.into_iter().map(str::to_owned).collect()));
    }
    strategy
}
...
fn current_catalog(strategy: &Strategy, refresh: bool) -> Result<Catalog> {
    if !refresh {
        if let Ok(catalog) = Catalog::load() {
            if catalog.matches_strategy(strategy) { return Ok(catalog); }
        }
    }
...
fn validate_intent(strategy: &Strategy) -> Result<()> {
    ...
    if strategy.mixed_mode == InboundMode::Rule {
        if matches!(strategy.routing_profile, RoutingProfile::Chinadirect | RoutingProfile::Gfwlist)
            || strategy.rule_sets.iter().any(|set| crate::gfw::gfw_group(&set.via).is_some()) {
            bail!("此测试通道尚未加载地区 IP 库或 GFWList。请选择未匹配走组或直连。");
        }
        if strategy.rule_sets.iter().any(|set| set.matchers.iter().any(|matcher| matcher.kind == "app")) {
...
fn activate(strategy: &Strategy, catalog: &Catalog) -> Result<()> {
    ...
    let entrance = if same_port { None } else {
        let listener = TcpListener::bind(("127.0.0.1", strategy.mixed_port))
            .with_context(|| format!("混合端口 {} 已被占用，现有代理保持运行", strategy.mixed_port))?;
    ...
    if let Some(previous) = previous { previous.stopped.store(true, Ordering::Release); }
    drop(previous_entrance);
    if !cfg!(test) { start_probes(&candidate); }
    Ok(())
}
...
fn prepare(strategy: &Strategy, catalog: &Catalog) -> Result<Arc<Runtime>> {
    ...
    #[cfg(not(unix))] fs::create_dir(&directory)?;
...
fn prepare_in(strategy: &Strategy, catalog: &Catalog, directory: PathBuf) -> Result<Arc<Runtime>> {
    ...
    for (index, node) in catalog.nodes.iter().enumerate() {
        ...
        let outbound = match nodes::render(node, &tag) {
            ...
            Err(error) => { warnings.push(format!("{}：{error}", node.name)); continue; }
        };
        let check = directory.join("node-check.json");
        paths::atomic_write(&check, &serde_json::to_vec(&json!({"outbounds":[outbound.clone()]}))?)?;
        let validation = Command::new(&binary).args(["run","-test","-config"]).arg(&check).output()?;
        ...
        lanes.insert(node.name.clone(), Lane { address, username, password });
    ...
    let binary = xray_binary()?;
    let validation = Command::new(&binary).args(["run","-test","-config"]).arg(&path).output()?;
    if !validation.status.success() {
        // Core diagnostics may contain credentials. Keep them in the private session file.
        paths::atomic_write(&directory.join("validation.log"), &[validation.stdout, validation.stderr].concat())?;
        bail!("节点配置未通过 Xray 内核验证，现有连接未改变");
    }
    drop(reserved_ports);
    paths::atomic_write(&directory.join("core.log"), b"")?;
    let log = fs::OpenOptions::new().append(true).open(directory.join("core.log"))?;
    let child = Command::new(binary).args(["run","-config"]).arg(path)
        .stdin(Stdio::null()).stdout(Stdio::from(log.try_clone()?)).stderr(Stdio::from(log)).spawn()?;
    let runtime = Arc::new(Runtime {
        strategy: RwLock::new(strategy.clone()), catalog: accepted,
        health: RwLock::new(HashMap::new()), lanes, child: Mutex::new(child),
        stopped: AtomicBool::new(false), generation: AtomicU64::new(next_generation()),
        warnings, directory, probe_lock: Mutex::new(()),
    });
    let deadline = Instant::now() + Duration::from_secs(5);
    loop {
        if !runtime.core_alive() { bail!("Xray 在启动时退出，现有配置未改变"); }
        if runtime.lanes.values().all(|lane| std::net::TcpStream::connect_timeout(&lane.address, Duration::from_millis(100)).is_ok()) {
            break;
        }
        if Instant::now() >= deadline { bail!("Xray 私有入口未就绪，现有配置未改变"); }
...
pub fn xray_binary() -> Result<PathBuf> {
    let binary = std::env::var_os("XRAY_BINARY").map(PathBuf::from).unwrap_or_else(paths::bundled_xray);
    if !binary.is_file() { bail!("找不到 Xray 内核，请重新下载完整的 Xray 测试应用"); }
    Ok(binary)
}

impl Runtime {
    fn core_alive(&self) -> bool {
        !self.stopped.load(Ordering::Acquire) && matches!(self.child.lock().expect("Xray child").try_wait(), Ok(None))
    }
    fn dial(&self, host: &str, port: u16) -> Result<Dialed> {
        if !self.core_alive() { bail!("Xray 内核已退出，请重新连接"); }
        let decision = policy::decide(&self.strategy.read().expect("strategy"), &self.catalog,
            &self.health.read().expect("health"), host, port, None);
        ...
        Ok(Dialed { stream, chain: decision.chain.join(" → "), rule: decision.rule })
    }
    fn probe(&self) {
        let Ok(_lock) = self.probe_lock.try_lock() else { return; };
            ...
            for chunk in lanes.chunks(lanes.len().div_ceil(8).max(1)) {
                scope.spawn(move || {
                    for (name, lane) in chunk {
                        ...
                        let output = Command::new("/usr/bin/curl").args(["-q","--silent","--output","/dev/null",
                            "--write-out","%{http_code}","--connect-timeout","2","--max-time","3","--noproxy","",
                            "--proxy", &format!("socks5h://{}",lane.address),"--proxy-user", &credentials, PROBE_URL])
                            .env_remove("ALL_PROXY").env_remove("HTTPS_PROXY").env_remove("HTTP_PROXY").output();
                        let alive = output.is_ok_and(|result| result.status.success() && result.stdout == b"204");
                        let mut health = self.health.write().expect("health");
                        let item = health.entry((*name).clone()).or_default();
                        if alive { item.delay_ms = Some(start.elapsed().as_millis().min(u32::MAX as u128) as u32); item.failures = 0; }
                        else { item.delay_ms = None; item.failures = item.failures.saturating_add(1); }
                    }
                });
            }
        });
    ...
    std::thread::spawn(move || loop {
        {
            ...
            if runtime.stopped.load(Ordering::Acquire) { return; }
            runtime.probe();
        }
        for _ in 0..30 {
            std::thread::sleep(Duration::from_secs(1));
            if weak.upgrade().is_none_or(|runtime| runtime.stopped.load(Ordering::Acquire)) { return; }
        }
    });
}
...
pub fn select_proxy(identity: &RuntimeIdentity, group: &str, name: &str) -> Result<()> {
    let _operation = OPERATION.lock().expect("Xray operation");
    let runtime = active()?;
    if runtime.generation.load(Ordering::Acquire) != identity.generation { bail!("运行配置已变化，请刷新后重试"); }
    let mut strategy = runtime.strategy.read().expect("strategy").clone();
    let candidates = policy::groups(&strategy, &runtime.catalog, &runtime.health.read().expect("health"));
    let live = candidates.iter().find(|live| live.name == group).context("节点组不存在")?;
    if !name.is_empty() && !live.members.iter().any(|member| member.name == name) { bail!("所选节点不是该组的可用成员"); }
    let saved = Strategy::load()?;
    let saved_name = if group == GLOBAL_GROUP { saved.global_selected.as_str() } else {
        saved.groups.iter().find(|item| item.name == group).context("节点组已被删除")?.selected.as_str()
    };
    if saved_name != name { bail!("保存的节点选择已变化，请重新选择"); }
    if group == GLOBAL_GROUP { strategy.global_selected = name.into(); }
    else { strategy.groups.iter_mut().find(|item| item.name == group).context("节点组不存在")?.selected = name.into(); }
...
pub fn disconnect() -> Result<()> {
    ...
    if let Some(runtime) = &previous.runtime { runtime.stopped.store(true, Ordering::Release); }
    if let Some(entrance) = &previous.entrance { entrance.close_all()?; }
    drop(previous);
    Ok(())
}

pub fn is_running() -> bool { active().is_ok_and(|runtime| runtime.core_alive()) }
pub fn wanted() -> bool { service().lock().expect("Xray service").runtime.is_some() }
pub fn sync_wanted_on_launch() -> Result<()> { Ok(()) }
pub fn runtime_identity() -> Option<RuntimeIdentity> {
    let runtime = active().ok()?;
    if !runtime.core_alive() { return None; }
    let port = runtime.strategy.read().ok()?.mixed_port;
    Some(RuntimeIdentity { generation:runtime.generation.load(Ordering::Acquire),mixed_port:port })
}
...
pub fn status() -> Result<XrayStatus> {
    ...
    let Some(runtime) = &state.runtime else {
        return Ok(XrayStatus { wanted:false,running:false,ready:false,generation:None,mixed_port:None,current:String::new(),warnings:vec![],note:None });
    };
    let running = runtime.core_alive();
    let strategy = runtime.strategy.read().expect("strategy");
    let decision = policy::decide(&strategy,&runtime.catalog,&runtime.health.read().expect("health"),"example.invalid",443,None);
    let current = decision.chain.last().cloned().unwrap_or_default();
    Ok(XrayStatus { wanted:true,running,ready:running && state.entrance.is_some(),
        generation:Some(runtime.generation.load(Ordering::Acquire)),mixed_port:Some(strategy.mixed_port),
        current,warnings:runtime.warnings.clone(),note:(!running).then(||"Xray 已退出，请重新连接".into()) })
}
...
pub fn traffic() -> Result<controller::TrafficSnapshot> {
    let entrance = service().lock().expect("Xray service").entrance.clone().context("尚未连接")?;
...
pub fn test_group_delay(group: &str) -> Result<HashMap<String,u32>> {
    let runtime = active()?;
    runtime.probe();
    Ok(groups()?.into_iter().find(|item|item.name == group).context("节点组不存在")?.members.into_iter()
        .filter_map(|member|member.delay.map(|delay|(member.name,delay))).collect())
}
fn save_applied(runtime: &Runtime) -> Result<()> {
    save_snapshot(runtime,&runtime.strategy.read().expect("strategy"),runtime.generation.load(Ordering::Acquire))
}
fn save_snapshot(runtime: &Runtime, strategy: &Strategy, generation:u64) -> Result<()> {
    paths::atomic_write(&paths::xray_runtime_state_path()?,&serde_json::to_vec(&SavedRuntime {
        strategy:strategy.clone(),generation,core_pid:runtime.child.lock().expect("child").id(),
    })?)
}
fn next_generation() -> u64 {
    let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos().min(u64::MAX as u128) as u64;
    GENERATION.fetch_max(now,Ordering::AcqRel).max(now).saturating_add(1)
}
