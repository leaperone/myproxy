...
pub fn parse_links(text: &str) -> Result<Vec<Value>> {
    if text.len() > 4 * 1024 * 1024 { bail!("分享内容超过4MB"); }
    ...
    if !lines.iter().any(|line| is_supported_scheme(line)) {
        if let Ok(decoded) = decode_b64(text.trim()) {
            let decoded = String::from_utf8(decoded).map_err(|_| anyhow::anyhow!("分享内容不是文本链接"))?;
...
fn parse_trojan(body: &str) -> Result<Value> {
    ...
    if query.get("security").is_some_and(|v| v != "tls" && v != "none") {
...
fn parse_vmess(body: &str) -> Result<Value> {
    ...
    let json: serde_json::Value = serde_json::from_slice(&decoded).map_err(|_| anyhow::anyhow!("VMess 分享内容不是有效配置"))?;
    let object = json.as_object().ok_or_else(|| anyhow::anyhow!("VMess 分享内容不是对象"))?;
    ...
    let mut node = base_node("vmess", &host, port, json_string(object, "ps").as_deref().unwrap_or(""))?;
    put(&mut node, "uuid", uuid);
    if let Some(cipher) = json_string(object, "scy") { put(&mut node, "cipher", cipher); }
    let network = json_string(object, "net").unwrap_or_else(|| "tcp".into());
    put(&mut node, "network", network.clone());
    if json_string(object, "tls").is_some_and(|v| v != "none" && !v.is_empty()) { put(&mut node, "tls", "true"); }
    if let Some(sni) = json_string(object, "sni") { put(&mut node, "servername", sni); }
    let mut opts = Mapping::new();
    if let Some(path) = json_string(object, "path") { put(&mut opts, "path", path); }
    if let Some(host_header) = json_string(object, "host") { put(&mut opts, "headers", mapping_one("Host", host_header)); }
    if network.eq_ignore_ascii_case("ws") && !opts.is_empty() { put(&mut node, "ws-opts", opts); }
    if network.eq_ignore_ascii_case("grpc") {
        if let Some(service) = json_string(object, "path") { put(&mut node, "grpc-opts", mapping_one("grpc-service-name", service)); }
    }
    if let Some(name) = json_string(object, "type") {
        if name != "none" && !name.is_empty() { bail!("VMess transport type 不受支持"); }
    }
    Ok(Value::Mapping(node))
}
...
fn parse_ss(body: &str) -> Result<Value> {
    ...
    if let Some(key) = query_values.keys().next() {
        if matches!(key.as_str(), "plugin" | "plugin-opts" | "obfs" | "obfs-password") {
            bail!("Shadowsocks plugin/obfs 未映射到 Xray");
        }
        bail!("Shadowsocks 链接包含未支持的参数");
    }
    let decoded = decode_b64(main).ok();
    let payload = decoded.as_deref().and_then(|v| std::str::from_utf8(v).ok()).unwrap_or(main);
    let at = payload.rfind('@').ok_or_else(|| anyhow::anyhow!("Shadowsocks 分享内容格式无效"))?;
    let raw_userinfo = &payload[..at];
    let decoded_userinfo = if raw_userinfo.contains(':') { raw_userinfo.to_string() } else {
        String::from_utf8(decode_b64(raw_userinfo)?).map_err(|_|anyhow::anyhow!("Shadowsocks 认证信息无效"))?
    };
    let userinfo = decoded_userinfo.as_str();
    let endpoint = payload[at + 1..].trim_end_matches('/');
    let colon = userinfo.find(':').ok_or_else(|| anyhow::anyhow!("Shadowsocks 分享内容缺少认证信息"))?;
...
fn apply_common_query(node: &mut Mapping, query: &HashMap<String, String>, vless: bool) -> Result<()> {
    for key in query.keys() {
        if matches!(key.as_str(), "allowInsecure" | "insecure" | "skip-cert-verify") {
            bail!("不安全的证书校验参数被拒绝");
        }
        if !matches!(key.as_str(), "type" | "security" | "sni" | "servername" | "alpn" | "fp" | "flow" | "path" | "host" | "serviceName" | "service-name" | "pbk" | "sid" | "spx" | "encryption" | "headerType") {
            bail!("链接包含未支持的参数");
        }
    }
    if query.get("encryption").is_some_and(|v|v != "none") || query.get("headerType").is_some_and(|v|v != "none") {
        bail!("链接的加密或伪装参数尚不支持");
    }
    if let Some(security) = query.get("security") {
        match security.as_str() {
            ...
            "reality" => {
                let pbk = query.get("pbk").ok_or_else(|| anyhow::anyhow!("Reality 链接缺少 public-key"))?;
    ...
    if let Some(value) = query.get("sni").or_else(|| query.get("servername")) { put(node, "servername", value.as_str()); }
    if let Some(value) = query.get("alpn") { put(node, "alpn", yaml_sequence(value.split(',').map(str::to_owned))); }
    if let Some(value) = query.get("fp") { put(node, "client-fingerprint", value.as_str()); }
    if let Some(value) = query.get("flow") { put(node, "flow", value.as_str()); }
    let network = query.get("type").map(String::as_str).unwrap_or("tcp");
    match network {
        "tcp" | "raw" => {}
        "ws" => { put(node, "network", "ws"); let mut opts = Mapping::new(); if let Some(path) = query.get("path") { put(&mut opts, "path", path.as_str()); } if let Some(host) = query.get("host") { put(&mut opts, "headers", mapping_one("Host", host.clone())); } put(node, "ws-opts", opts); }
        "grpc" => { put(node, "network", "grpc"); let mut opts = Mapping::new(); if let Some(service) = query.get("serviceName").or_else(|| query.get("service-name")) { put(&mut opts, "grpc-service-name", service.as_str()); } put(node, "grpc-opts", opts); }
        "xhttp" | "splithttp" => { put(node, "network", "xhttp"); let mut opts = Mapping::new(); if let Some(path) = query.get("path") { put(&mut opts, "path", path.as_str()); } if let Some(host) = query.get("host") { put(&mut opts, "host", host.as_str()); } put(node, "xhttp-opts", opts); }
...
fn base_node(kind: &str, host: &str, port: u16, name: &str) -> Result<Mapping> {
    if host.is_empty() { bail!("链接缺少服务器地址"); }
...
fn parse_authority(value: &str, user_required: bool) -> Result<(String, String, u16)> {
    let at = value.rfind('@').ok_or_else(|| anyhow::anyhow!("链接缺少认证信息"))?;
    let user = percent_decode(&value[..at])?;
    if user_required && user.is_empty() { bail!("链接认证信息为空"); }
...
fn parse_host_port(value: &str) -> Result<(String, u16)> {
    if let Some(rest) = value.strip_prefix('[') {
        let end = rest.find(']').ok_or_else(|| anyhow::anyhow!("IPv6 地址格式无效"))?;
        let host = percent_decode(&rest[..end])?;
        host.parse::<std::net::Ipv6Addr>().map_err(|_|anyhow::anyhow!("IPv6 地址格式无效"))?;
        let port = rest[end + 1..].strip_prefix(':').ok_or_else(|| anyhow::anyhow!("链接缺少端口"))?.parse().map_err(|_| anyhow::anyhow!("端口无效"))?;
        if port == 0 { bail!("端口无效"); }
        return Ok((host.into(), port));
    }
    let (host, port) = value.rsplit_once(':').ok_or_else(|| anyhow::anyhow!("链接缺少端口"))?;
    let port = port.parse().map_err(|_| anyhow::anyhow!("端口无效"))?;
    if host.is_empty() || host.contains(':') || host.chars().any(|c| c.is_whitespace() || c.is_control() || "/@#\\".contains(c)) || port == 0 { bail!("服务器地址或端口无效"); }
...
fn query_map(query: &str) -> Result<HashMap<String, String>> {
    ...
    for pair in query.split('&') {
        let (key, value) = pair.split_once('=').ok_or_else(|| anyhow::anyhow!("链接参数格式无效"))?;
        let key = percent_decode(key)?;
        let value = percent_decode(value)?;
        if key.is_empty() || result.insert(key, value).is_some() { bail!("链接参数重复或为空"); }
    }
    Ok(result)
}
...
fn percent_decode(value: &str) -> Result<String> {
    ...
    while i < bytes.len() {
        if bytes[i] == b'%' {
            if i + 2 >= bytes.len() { bail!("链接转义格式无效"); }
...
fn decode_b64(value: &str) -> Result<Vec<u8>> {
    ...
    base64::engine::general_purpose::STANDARD.decode(padded).map_err(|_| anyhow::anyhow!("链接内容不是有效 Base64"))
}

fn json_string(object: &serde_json::Map<String, serde_json::Value>, key: &str) -> Option<String> {
    object.get(key).and_then(|value| value.as_str().map(ToOwned::to_owned).or_else(|| value.as_u64().map(|v| v.to_string())))
}

fn json_port(object: &serde_json::Map<String, serde_json::Value>, key: &str) -> Option<u16> {
    json_string(object, key)?.parse().ok().filter(|port| *port > 0)
}
...
mod tests {
    ...
    fn parses_vless_ipv6_and_escaped_fields() {
        ...
        assert_eq!(map_value(node, "server").and_then(Value::as_str), Some("2001:db8::1"));
        assert_eq!(map_value(node, "name").and_then(Value::as_str), Some("Japan One"));
        assert_eq!(map_value(node, "tls").and_then(Value::as_str), Some("true"));
    }

    #[test]
    fn parses_sip002_and_vmess_padding() {
        let ss_payload = base64::engine::general_purpose::STANDARD.encode("aes-128-gcm:pa%40ss@[2001:db8::2]:8443");
        ...
        let encoded = base64::engine::general_purpose::STANDARD.encode(serde_json::to_vec(&json).unwrap()).trim_end_matches('=').to_owned();
    ...
    fn rejects_unsupported_security_transport_and_credentials_in_errors() {
        ...
        assert!(parse_links("ss://YWVzLTEyOC1nY206cGFzc0BleGFtcGxlLmNvbTo0NDM/?plugin=obfs").is_err());
