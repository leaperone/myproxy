...
fn vmess(raw: &YamlValue, address: &str, port: u16) -> Result<Value> {
    ...
    let security = raw_string(raw, "cipher").unwrap_or_else(|| "auto".into()).to_ascii_lowercase();
...
fn vless(raw: &YamlValue, address: &str, port: u16) -> Result<Value> {
    ...
    if let Some(flow) = raw_string(raw, "flow") {
        if !flow.is_empty() { user.insert("flow".into(), Value::String(flow)); }
    }
...
fn stream_settings(raw: &YamlValue, kind: &str) -> Result<Option<Value>> {
    ...
    if tls {
        ...
        if let Some(alpn) = raw_string_list(raw, "alpn") { security.insert("alpn".into(), json!(alpn)); }
        if let Some(fp) = raw_string(raw, "client-fingerprint") { security.insert("fingerprint".into(), fp.into()); }
        ...
        if let Some(reality) = reality {
            ...
            if public_key_bytes.len() != 32 { bail!("Reality public-key 必须解码为 32 字节"); }
            if !short_id.chars().all(|c| c.is_ascii_hexdigit()) || short_id.len() % 2 != 0 || short_id.len() > 16 { bail!("Reality short-id 必须是最多 8 字节的偶数位十六进制"); }
...
fn ws_settings(raw: &YamlValue) -> Value {
    ...
    let mut out = Map::new();
    out.insert("path".into(), raw_map_string_ref(&opts, "path").unwrap_or_else(|| "/".into()).into());
    if let Some(headers) = opts.as_ref().and_then(|m| m.get(YamlValue::String("headers".into()))).and_then(YamlValue::as_mapping) {
        let mut map = BTreeMap::new();
        for (key, value) in headers { if let (Some(k), Some(v)) = (key.as_str(), value.as_str()) { map.insert(k.to_string(), v.to_string()); } }
...
fn xhttp_settings(raw: &YamlValue) -> Value {
    ...
    let mut out = Map::new();
    out.insert("path".into(), raw_map_string_ref(&opts, "path").unwrap_or_else(|| "/".into()).into());
    if let Some(v) = raw_map_string_ref(&opts, "host") { out.insert("host".into(), v.into()); }
    if let Some(v) = raw_map_string_ref(&opts, "mode") { out.insert("mode".into(), v.into()); }
    Value::Object(out)
}

fn reject_unimplemented_features(raw: &YamlValue) -> Result<()> {
    for key in ["plugin", "plugin-opts", "obfs", "obfs-opts", "obfs-password"] {
        if raw_value(raw, key).is_some() { bail!("节点字段 {key} 未映射到 Xray，已拒绝以避免静默改变传输"); }
    }
    Ok(())
}
...
fn validate_pin(value: &str) -> Result<String> {
    ...
    if value.split(',').all(|item| item.trim().is_empty()) { bail!("pinnedPeerCertSha256 不能为空"); }
    Ok(value.trim().into())
}

fn required_uuid(raw: &YamlValue, key: &str) -> Result<String> { let value = required_string(raw, key)?; Uuid::parse_str(&value).context("UUID 格式无效")?; Ok(value) }
fn required_port(raw: &YamlValue) -> Result<u16> { let value = raw_u64(raw, "port").context("缺少服务器端口")?; u16::try_from(value).ok().filter(|p| *p > 0).context("服务器端口必须为 1-65535") }
fn required_string(raw: &YamlValue, key: &str) -> Result<String> { let value = raw_string(raw, key).context(format!("缺少 {key}"))?; if value.trim().is_empty() { bail!("{key} 不能为空"); } Ok(value) }
fn raw_value<'a>(raw: &'a YamlValue, key: &str) -> Option<&'a YamlValue> { raw.as_mapping()?.get(YamlValue::String(key.to_string())) }
fn raw_string(raw: &YamlValue, key: &str) -> Option<String> { raw_value(raw, key).and_then(|v| match v { YamlValue::String(s) => Some(s.clone()), YamlValue::Number(n) => Some(n.to_string()), _ => None }) }
fn raw_truthy(raw: &YamlValue, key: &str) -> bool { raw_value(raw, key).and_then(|v| match v { YamlValue::Bool(b) => Some(*b), YamlValue::String(s) => Some(s.eq_ignore_ascii_case("true")), _ => None }).unwrap_or(false) }
fn raw_u64(raw: &YamlValue, key: &str) -> Option<u64> { raw_value(raw, key).and_then(|v| match v { YamlValue::Number(n) => n.as_u64(), YamlValue::String(s) => s.parse().ok(), _ => None }) }
fn raw_u32(raw: &YamlValue, key: &str) -> Option<u32> { raw_u64(raw, key).and_then(|v| u32::try_from(v).ok()) }
fn raw_map(raw: &YamlValue, key: &str) -> Option<serde_yaml::Mapping> { raw_value(raw, key)?.as_mapping().cloned() }
fn raw_map_string(map: &serde_yaml::Mapping, key: &str) -> Option<String> { map.get(YamlValue::String(key.to_string())).and_then(|v| match v { YamlValue::String(s) => Some(s.clone()), YamlValue::Number(n) => Some(n.to_string()), _ => None }) }
fn raw_map_string_ref(map: &Option<serde_yaml::Mapping>, key: &str) -> Option<String> { map.as_ref().and_then(|m| raw_map_string(m, key)) }
fn raw_string_list(raw: &YamlValue, key: &str) -> Option<Vec<String>> { match raw_value(raw, key)? { YamlValue::Sequence(a) => Some(a.iter().filter_map(|v| raw_yaml_string(v)).collect()), v => raw_yaml_string(v).map(|s| vec![s]) } }
fn raw_yaml_string(value: &YamlValue) -> Option<String> { match value { YamlValue::String(s) => Some(s.clone()), YamlValue::Number(n) => Some(n.to_string()), _ => None } }
...
mod tests {
    ...
    fn xray_binary_schema_hook_is_optional() {
        ...
        for kind in ["ss", "vmess", "vless", "trojan", "socks5", "http", "hysteria2"] {
            let mut fixture = node(kind);
            if kind == "hysteria2" {
                fixture.raw.as_mapping_mut().unwrap().remove(YamlValue::String("password".into()));
                fixture.raw.as_mapping_mut().unwrap().remove(YamlValue::String("tls".into()));
            }
            ...
            let path = std::env::temp_dir().join(format!("myproxy-xray-node-{}.json", Uuid::new_v4()));
        ...
        for network in ["ws", "grpc", "xhttp", "reality"] {
            ...
            match network {
                "ws" => {
                    map.insert(YamlValue::String("network".into()), YamlValue::String("ws".into()));
                    map.insert(YamlValue::String("ws-opts".into()), serde_yaml::from_str("path: /fixture\n").unwrap());
                }
                "grpc" => {
                    map.insert(YamlValue::String("network".into()), YamlValue::String("grpc".into()));
                    map.insert(YamlValue::String("grpc-opts".into()), serde_yaml::from_str("grpc-service-name: fixture\n").unwrap());
                }
                "xhttp" => {
                    map.insert(YamlValue::String("network".into()), YamlValue::String("xhttp".into()));
                    map.insert(YamlValue::String("xhttp-opts".into()), serde_yaml::from_str("path: /fixture\nmode: auto\n").unwrap());
                }
                "reality" => {
            ...
            let path = std::env::temp_dir().join(format!("myproxy-xray-transport-{}.json", Uuid::new_v4()));
            std::fs::write(&path, serde_json::to_vec(&config).unwrap()).unwrap();
            let result = std::process::Command::new(&bin).args(["run", "-test", "-config"]).arg(&path).output();
            let _ = std::fs::remove_file(&path);
            let result=result.expect("spawn XRAY_BINARY");
