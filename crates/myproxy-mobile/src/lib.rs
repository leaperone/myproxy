...
fn command(request: &str) -> Result<String> {
    ...
    match op {
        ...
        "load" => { let text=req.get("document").and_then(Value::as_str).context("缺少 document")?; let loaded: Document=serde_json::from_str(text).context("配置文件格式无效")?; loaded.strategy.validate()?; loaded.catalog.validate()?; guard.strategy=loaded.strategy; guard.catalog=loaded.catalog; guard.revision+=1; Ok(ok(guard.snapshot())) },
        "export" => Ok(ok(json!({"document":serde_json::to_string(&Document{strategy:guard.strategy.clone(),catalog:guard.catalog.clone()})?}))),
        ...
        "import" => { let text=req.get("text").and_then(Value::as_str).context("缺少 text")?; let source=req.get("name").and_then(Value::as_str).unwrap_or("导入节点"); let values=myproxy_core::parse_links(text)?; let mut count=0; for value in values { let node=parse_node(value,source,"导入节点")?; guard.catalog.nodes.push(node); count+=1; } guard.revision+=1; Ok(ok(json!({"imported":count,"snapshot":guard.snapshot()}))) },
