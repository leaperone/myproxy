...
把现有 `myproxy.log` 做成应用级分级日志：Info / Warning / Error 始终可见、可打开；Host 与 Network Extension 生命周期写入同一文件。

## 范围

- 扩展 `src/log.rs`：统一行格式、文件锁、从文件读尾
- 设置页独立「日志」面板（分级说明、路径、Finder、尾部着色）
- NetworkShared 文件日志；Host / NE 启停与失败打 info/warn/error
- AGENTS 补一句：设置页始终展示 info/warn/error，Swift 侧写同一文件
...
- Host 与 NE 是不同进程，必须 flock 后追加同一 `myproxy.log`
...
- Host 启用/停用/审批/失败与 NE start/stop/失败写入同一 `myproxy.log` 且格式一致
- debug/trace 仍仅开发者模式
...
| Swift 写同一 `myproxy.log` + flock | NE 无法调 Rust FFI；UI/`myproxyctl log` 只读该文件 |
