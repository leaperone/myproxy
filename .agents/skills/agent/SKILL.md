...
The default runtime channel is `mihomo`. The isolated Xray test channel is
opt-in and stores its selector in `backend.json`, outside `strategy.json`:

```sh
myproxyctl --json backend
myproxyctl --json backend xray
myproxyctl --json connect
```

Run `myproxyctl disconnect` before switching back with `myproxyctl backend
mihomo`. Xray uses the configured Mixed port for SOCKS5 and the next port for
HTTP. The Xray binary must be bundled with the test package.
